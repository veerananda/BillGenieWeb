        ...
        <div className="px-6 py-5 space-y-6">
          ...
          <div className="rounded-xl border border-primary/15 bg-primary/5 px-4 py-4">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-wider text-primary">Current Plan</p>
                <p className="mt-0.5 text-xl font-bold text-gray-900">{planName}</p>
                <p className="mt-0.5 text-sm text-gray-500">₹{monthlyPrice}/month</p>
              </div>
              {subEnd && (
                <div className="shrink-0 rounded-lg border border-primary/15 bg-white px-3 py-2 text-right">
                  <div className="flex items-center justify-end gap-1 text-xs text-gray-400">
                    <Calendar className="h-3 w-3" />
                    <span>Renews {formatDate(subEnd)}</span>
                  </div>
                  {daysLeft !== null && (
                    <p className={`mt-0.5 text-sm font-bold ${daysColor}`}>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
              {planRows.map((row) => (
                <div key={row.label} className="rounded-xl border border-gray-100 bg-gray-50 px-3.5 py-3">
                  <p className="text-xs text-gray-400">{row.label}</p>
                  <p className="mt-0.5 text-sm font-bold text-gray-900">{row.value}</p>
                </div>
              ))}
            </div>
          </div>
